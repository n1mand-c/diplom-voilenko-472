...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Route based on role (usually ROLE_USER for new registers)
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative z-10 flex flex-col items-center">
        <PasswordCharacters inputId="register-password" isPasswordHidden={!showPassword} />
        <div className="w-full bg-white/5 border border-white/10 p-8 rounded-3xl shadow-2xl backdrop-blur-sm">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-white mb-2 text-center">Реєстрація</h1>
            <p className="text-white/50 text-sm text-center">Створіть акаунт для бронювання</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="text-white/50 text-sm mb-1.5 block">Логін</label>
            <input
              type="text"
              required
              value={form.username}
          ...
          <div>
            <label className="text-white/50 text-sm mb-1.5 block">Підтвердження паролю</label>
            <input
              type={showPassword ? "text" : "password"}
              required
        ...
        <div className="mt-6 text-center text-sm text-white/50">
          Вже маєте акаунт? <Link href="/login" className="text-[#C8102E] hover:underline">Увійти</Link>
        </div>
        </div>
      </div>
    </div>
