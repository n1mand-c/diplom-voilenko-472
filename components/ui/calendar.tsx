...
export function Calendar({
  ...
  const defaultClassNames = {
    ...
    day: "size-(--cell-size) text-sm py-px text-white/70 hover:bg-white/10 hover:text-white rounded-md",
    day_button: cn(
      buttonClassNames,
    ...
    dropdown: "absolute inset-0 opacity-0 [color-scheme:dark] cursor-pointer",
    dropdown_root:
      "relative flex items-center gap-1 rounded-lg border border-white/20 bg-white/10 px-2 h-8 text-white text-sm [&_svg]:size-3 [&_svg]:opacity-60 hover:bg-white/15 transition-colors",
    dropdowns:
    ...
    weekday:
      "size-(--cell-size) p-0 text-xs font-medium text-white/40",
    selected: "bg-[#C8102E] text-white hover:bg-[#a00d25] hover:text-white focus:bg-[#C8102E] focus:text-white",
  };
  ...
  const dayPickerProps = {
    className: cn(
      "w-fit [--cell-size:--spacing(10)] sm:[--cell-size:--spacing(9)] p-3",
      className,
    ),
